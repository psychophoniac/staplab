...
class OutputSplitterModule(OutputModule):
	def __init__(self,outstreams=[],mID=-1):
		self.os = outstreams
		OutputModule.__init__(self,mID)

	def out(self,(timestamp, moduleName, moduleID, data)):
		for om in self.os:
			om.out((timestamp, moduleName, moduleID, data))

	def outLog(self,(timestamp, moduleName, moduleID, data)):
		for om in self.os:
			om.outLog((timestamp, moduleName, moduleID, data))

class GraphOutputModule(OutputModule):
	def __init__(self,graph=None,mID=-1):
		OutputModule.__init__(self,mID)
		self.g = graph
		self.running = False
